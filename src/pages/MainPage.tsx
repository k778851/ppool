  ...
  const filtered = useMemo(() => {
    return MOCK_RIDES.filter(ride => {
      ...
      if (activeFilter === 'available') {
        if (ride.status === 'FULL' || ride.current_seats >= ride.max_seats) return false
      }
      if (activeFilter === 'female') {
        if (ride.gender_preference !== 'SAME_ONLY') return false
      }
      return true
      ...
      <div className="hero-header">
        ...
        <div className="hero-car-bg">
          <svg width="120" height="72" viewBox="0 0 120 72" fill="none">
            <rect x="8" y="28" width="96" height="30" rx="10"
              fill="var(--mint)" stroke="rgba(255,255,255,0.6)" strokeWidth="1.5" />
            <rect x="20" y="12" width="68" height="28" rx="8"
              fill="var(--mint)" stroke="rgba(255,255,255,0.6)" strokeWidth="1.5" />
            ...
            <path d="M100 36 L112 36 L116 44 L112 54 L100 54"
              fill="var(--driver)" stroke="rgba(255,255,255,0.5)" strokeWidth="1" />
          </svg>
        </div>
        ...
        <div className="hero-logo-row">
          ...
          <button className="hero-bell-btn tap" aria-label="알림">
            <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
              <path d="M11 3 C7.5 3 5 5.5 5 9 L5 13 L3 16 L19 16 L17 13 L17 9 C17 5.5 14.5 3 11 3 Z"
                stroke="#fff" strokeWidth="1.8" strokeLinecap="round" fill="none" />
              <path d="M9 18 C9 19.5 10 20.5 11 20.5 C12 20.5 13 19.5 13 18"
                stroke="#fff" strokeWidth="1.8" strokeLinecap="round" fill="none" />
            </svg>
          </button>
        </div>
        ...
        <div className="hero-cta">
          <button
            ...
            onClick={() => MOCK_USER.is_driver ? navigate('/rides/new') : navigate('/my/driver')}
          >
            <div className="hero-cta-icon-wrap">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                <path d="M3 14 L3 10 L6 5 L14 5 L17 10 L17 14 M5 14 L5 16 M15 14 L15 16"
          ...
          <button
            className="hero-cta-btn hero-cta-rider tap"
            onClick={() => navigate('/')}
          >
            <div className="hero-cta-icon-wrap">
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                <circle cx="9" cy="6" r="3" stroke="#fff" strokeWidth="1.7" fill="none" />
                <path d="M2 17 C2 12.5 5 10 9 10 C13 10 16 12.5 16 17"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '0 16px 90px' }}>
          {filtered.length === 0 ? (
            ...
            filtered.map(ride => (
              <RideCard
                key={ride.id}
                ride={ride}
                isMyRide={ride.driver_id === MOCK_USER.id}
              />
            ))
          )}
          ...
          <div className="notice-card">
            <div style={{ flexShrink: 0, width: 44, height: 44, borderRadius: '50%', background: 'rgba(10,30,199,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                <path d="M4 5 L18 5 L18 15 L11 15 L8 18 L8 15 L4 15 Z"
                  stroke="var(--blue-hero-deep)" strokeWidth="1.8" fill="none" strokeLinejoin="round" />
