...
function StatBox({ value, label }: { value: string | number; label: string }) {
  return (
    <div style={{
      ...
      padding: '10px', textAlign: 'center', backdropFilter: 'blur(10px)',
    }}>
...
function ActiveRideCard({ rideId, status }: { rideId: string; status: string }) {
  ...
  return (
    <div className="my-ride-card tap" onClick={() => navigate(`/rides/${rideId}`)}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--ink-60)', fontWeight: 600 }}>
...
function MenuRow({ icon, label, count, right, last, onClick }: {
  icon: string; label: string; count?: number; right?: React.ReactNode; last?: boolean; onClick?: () => void
}) {
  return (
    <div className="my-menu-row tap" style={{ borderBottom: last ? 'none' : '1px solid var(--ink-10)' }} onClick={onClick}>
      ...
      {count !== undefined && (
        <span style={{ fontSize: 12, color: 'var(--ink-60)', fontWeight: 700, marginRight: 4 }}>{count}</span>
      )}
      {right}
    ...
    <div className="page-layout">
        ...
        <button style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
          <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
            ...
            <path d="M11 2 L11 5 M11 17 L11 20 M2 11 L5 11 M17 11 L20 11 M4.5 4.5 L6.5 6.5 M15.5 15.5 L17.5 17.5 M4.5 17.5 L6.5 15.5 M15.5 6.5 L17.5 4.5"
              stroke="var(--ink-80)" strokeWidth="1.7" strokeLinecap="round" />
          </svg>
        </button>
      </div>
      ...
      <div className="my-body">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '0 16px' }}>
              {activeRequests.map(req => (
                <ActiveRideCard
                  key={req.id}
                  rideId={req.ride_id}
                  status={req.status === 'APPROVED' ? '확정' : '대기중'}
                />
              ))}
              {myRides.map(ride => (
                <ActiveRideCard key={ride.id} rideId={ride.id} status="모집중" />
              ))}
            </div>
          </div>
        )}
        ...
        <div>
          ...
          <div className="my-menu-group">
            <MenuRow icon="🛡️" label="개인정보 처리방침" onClick={() => navigate('/privacy')} />
            <MenuRow icon="📑" label="이용약관" onClick={() => navigate('/terms')} />
            <MenuRow icon="📍" label="위치정보 이용약관" onClick={() => navigate('/location-terms')} />
            <MenuRow
              icon="💬" label="문의하기 (카카오톡 채널)"
              right={<span className="post-badge post-badge--mint" style={{ marginRight: 4 }}>연결</span>}
              onClick={() => {}}
            />
            <MenuRow icon="🚪" label="로그아웃" last onClick={() => navigate('/login')} />
          </div>
        </div>
