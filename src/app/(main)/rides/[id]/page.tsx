  ...
  return (
    <div className="page-layout">
      ...
      <div className="detail-topbar">
        ...
        <span className="detail-topbar-title">{isMyRide ? '내 게시글' : '운전자 게시글'}</span>
        <div style={{ width: 32 }} />
      </div>
      ...
      <div className="detail-hero" style={{ background: accentColor }}>
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          <span className="detail-hero-badge">{isMyRide ? '🚗 MY RIDE' : '🚗 DRIVER'}</span>
      ...
      <div className="detail-body">
        ...
        <section className="detail-section">
          ...
          <div className="detail-driver-card">
            <div className="detail-driver-avatar" style={{ background: driver?.gender === 'F' ? 'var(--rider-soft)' : 'var(--driver-soft)' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {driver?.gender && (
                  <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 999, background: driver.gender === 'F' ? '#FFE4EF' : '#E4EEFF', color: driver.gender === 'F' ? '#C0185A' : '#1A38F5' }}>
        ...
        {ride.notice && (
          <section className="detail-section">
            ...
            <div className="detail-notice-box" style={{ background: isMyRide ? 'var(--rider-soft)' : 'var(--driver-soft)', color: isMyRide ? 'var(--rider-ink)' : 'var(--driver-ink)' }}>
              {ride.notice}
            </div>
          </section>
        )}
        ...
        <div className="detail-legal">
          ...
          <div>본 서비스는 <b>지파 청년부 내부 유류비 분담</b> 목적의 매칭 서비스입니다.</div>
        </div>
      ...
      <div className="detail-bottom-cta">
        {isMyRide ? (
          <button className="detail-cta-primary tap" style={{ background: 'var(--blue-hero)' }} onClick={() => alert('출발 처리 (구현 예정)')}>🚗 출발하기</button>
        ) : requestState === 'approved' ? (
          <button className="detail-cta-ghost tap" onClick={() => setRequestState('none')}>✅ 탑승 확정 · 취소하기</button>
        ) : requestState === 'pending' ? (
          <button className="detail-cta-ghost tap" onClick={() => setRequestState('none')}>⏳ 신청 검토중 · 취소하기</button>
        ) : isFull ? (
          <button className="detail-cta-primary tap" disabled style={{ background: 'var(--ink-20)', color: 'var(--ink-60)', cursor: 'not-allowed' }}>마감된 게시글입니다</button>
        ) : (
          <button className="detail-cta-primary tap" style={{ background: accentColor }} onClick={() => setRequestState('pending')}>
